...
```{r tree model thing}
big <- rpart(traffic_volume ~ year + month + day + hournum + tod + weather_main + temp + holiday , data = training, method = "anova")
printcp(big)
plotcp(big)
rsq.rpart(big)
...
```
```{r testing}
X = testing[,c("weather_main","hournum","temp","day")]
X = testing[order(testing$traffic_volume),]
predictions = predict(big,X)
```

```{r prediction plot.. 0_0}
plot(X$hournum,X$traffic_volume,xlab='stuff',ylab="volume",tck=0.01)
