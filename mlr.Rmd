...
my_ts <- ts(vol_month$x, frequency=12, start = c(2015, 06), end = c(2017, 06))
plot(my_ts)
```
```{r}
...
my_ts_2 <- ts(vol_month_te$x, frequency=12, start = c(2016, 06), end = c(2018, 03))
plot(my_ts_2)
```
```{r}
...
n_type <- unique(traffic_dt[, type])

n_weeks <- floor(length(n_date)/7) - 2

lm_pred_weeks_1 <- sapply(0:(n_weeks-1), function(i)
  predWeekReg(traffic_dt[type == n_type[1]], n_date[((i*7)+1):((i*7)+7*2)]))

lm_err_mape_1 <- sapply(0:(n_weeks-1), function(i)
  mape(traffic_dt[(type == n_type[1] & date %in% n_date[(15+(i*7)):(21+(i*7))]), traffic_volume],
       lm_pred_weeks_1[, i+1]))
```

```{r}
datas <- data.table(value = c(as.vector(lm_pred_weeks_1),
                              traffic_dt[(type == n_type[1]) & (date %in% n_date[-c(1:14,365)]), traffic_volume]),
