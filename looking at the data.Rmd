...
```
```{r adding years}
df$year <- strftime(df$date_time, format="%Y")
```
```{r}
...
```
```{r}
df %>% filter(weather_main == "Clouds") %>% summarise(mean = mean(traffic_volume))
df %>% filter(weather_main == "Drizzle") %>% summarise(mean = mean(traffic_volume))
```
...
ggplot(df, aes(x = month, y = traffic_volume, group = month)) +
  geom_boxplot()
```
```{r}
